...
*/
Matrix_Graph::Matrix_Graph(int n, bool d) : Graph(n, d), edges(new double *[n])
{
  if (d)
  {
    for (int i = 0; i < n; i++)
...
{
  setEdgeValue(edge.get_source(), edge.get_destination(),
               edge.get_weight());
}
...
{
  return numeric_limits<double>::infinity() != getEdgeValue(source, dest);
}
...
void Matrix_Graph::iter_impl::advanceIndex()
{
  ...
  {
    index++;
  } while (index < parent->num_v && numeric_limits<double>::infinity() == parent->getEdgeValue(source, index));
}
...
bool Matrix_Graph::iter_impl::operator==(const Graph::iter_impl &other) const
{
  ...
  if (ptr_other == NULL)
    return false;
  return parent == ptr_other->parent && source == ptr_other->source && index == ptr_other->index;
}
