...
void Simpletron::parseInstruction(int InstructionRegister) {
    if (InstructionRegister < 0) {
        ...
        this->operationCode =
            InstructionRegister / INSTRUCTION_DIVISOR;         // Extract opcode (first 2 digits)
    ...
    switch (this->operationCode) {
        ...
        case 40:  // BRANCH (always jump, no increment)
            if (!checkMemoryBounds(this->operand)) {
                opcodeError(40, "Branch target out of bounds",
                            "Target address " + to_string(this->operand) +
                                " is out of memory bounds (0-9999)");
                return;
            }
        ...
        case 41:  // BRANCHNEG
            if (this->accumulator < 0) {
                if (!checkMemoryBounds(this->operand)) {
                    opcodeError(41, "Branch target out of bounds",
                                "Target address " + to_string(this->operand) +
                                    " is out of memory bounds (0-9999)");
                    return;
                }
        ...
        case 42:  // BRANCHZERO
            if (this->accumulator == 0) {
                if (!checkMemoryBounds(this->operand)) {
                    opcodeError(42, "Branch target out of bounds",
                                "Target address " + to_string(this->operand) +
                                    " is out of memory bounds (0-9999)");
                    return;
                }
        ...
        default:
            opcodeError(this->operationCode, "Invalid operation code",
                        "Operation code " + to_string(this->operationCode) + " is not recognized");
            break;
    }
}
...
void Simpletron::READ(int operand) {
    ...
    if (!checkMemoryBounds(operand)) {
        opcodeError(10, "Memory bounds violation",
                    "Operand " + to_string(operand) + " is out of memory bounds (0-9999)");
        return;
    }
    int input;
    cout << " ? ";
    ...
    if (checkOverflow(input)) {
        opcodeError(10, "Input overflow",
                    "Input value " + to_string(input) + " exceeds valid range (-999999 to 999999)");
        return;
    }
...
void Simpletron::WRITE(int operand) {
    // Cant access Negative memory
    if (!checkMemoryBounds(operand)) {
        opcodeError(11, "Memory bounds violation",
                    "Operand " + to_string(operand) + " is out of memory bounds (0-9999)");
        return;
    }
    cout << endl << "Output: " << this->memory[operand] << endl;
}

void Simpletron::LOAD(int operand) {
    // Cant access Negative memory
    if (!checkMemoryBounds(operand)) {
        opcodeError(20, "Memory bounds violation",
                    "Operand " + to_string(operand) + " is out of memory bounds (0-9999)");
        return;
    }
...
void Simpletron::LOADX(int operand) {
    // Cant access Negative memory
    if (!checkMemoryBounds(operand)) {
        opcodeError(22, "Memory bounds violation",
                    "Operand " + to_string(operand) + " is out of memory bounds (0-9999)");
        return;
    }
...
void Simpletron::LOADIDX() {
    ...
    if (!checkMemoryBounds(this->IndexRegister)) {
        opcodeError(23, "IndexRegister bounds violation",
                    "IndexRegister value " + to_string(this->IndexRegister) +
                        " is out of memory bounds (0-9999)");
        return;
    }
...
void Simpletron::STORE(int operand) {
    // Cant access Negative memory
    if (!checkMemoryBounds(operand)) {
        opcodeError(25, "Memory bounds violation",
                    "Operand " + to_string(operand) + " is out of memory bounds (0-9999)");
        return;
    }
...
void Simpletron::STOREIDX() {
    // Cant access Negative memory
    if (!checkMemoryBounds(this->IndexRegister)) {
        opcodeError(26, "IndexRegister bounds violation",
                    "IndexRegister value " + to_string(this->IndexRegister) +
                        " is out of memory bounds (0-9999)");
        return;
    }
...
void Simpletron::ADD(int operand) {
    // Cant access Negative memory
    if (!checkMemoryBounds(operand)) {
        opcodeError(30, "Memory bounds violation",
                    "Operand " + to_string(operand) + " is out of memory bounds (0-9999)");
        return;
    }
    if (checkOverflow(this->accumulator + this->memory[operand])) {
        opcodeError(30, "Arithmetic overflow",
                    "Result " + to_string(this->accumulator + this->memory[operand]) +
                        " exceeds valid range (-999999 to 999999)");
        return;
    }
    this->accumulator += this->memory[operand];
}
...
void Simpletron::ADDX() {
    // Cant access Negative memory
    if (!checkMemoryBounds(this->IndexRegister)) {
        opcodeError(31, "IndexRegister bounds violation",
                    "IndexRegister value " + to_string(this->IndexRegister) +
                        " is out of memory bounds (0-9999)");
        return;
    }
    ...
    if (checkOverflow(this->accumulator + this->memory[this->IndexRegister])) {
        opcodeError(31, "Arithmetic overflow",
                    "Result " + to_string(this->accumulator + this->memory[this->IndexRegister]) +
                        " exceeds valid range (-999999 to 999999)");
        return;
    }
    this->accumulator += this->memory[this->IndexRegister];
...
void Simpletron::SUBTRACT(int operand) {
    // Cant access Negative memory
    if (!checkMemoryBounds(operand)) {
        opcodeError(32, "Memory bounds violation",
                    "Operand " + to_string(operand) + " is out of memory bounds (0-9999)");
        return;
    }
    // Overflow
    if (checkOverflow(this->accumulator - this->memory[operand])) {
        opcodeError(32, "Arithmetic overflow",
                    "Result " + to_string(this->accumulator - this->memory[operand]) +
                        " exceeds valid range (-999999 to 999999)");
        return;
    }
...
void Simpletron::SUBTRACTX() {
    // Cant access Negative memory
    if (!checkMemoryBounds(this->IndexRegister)) {
        opcodeError(33, "IndexRegister bounds violation",
                    "IndexRegister value " + to_string(this->IndexRegister) +
                        " is out of memory bounds (0-9999)");
        return;
    }
    // Overflow
    if (checkOverflow(this->accumulator - this->memory[this->IndexRegister])) {
        opcodeError(33, "Arithmetic overflow",
                    "Result " + to_string(this->accumulator - this->memory[this->IndexRegister]) +
                        " exceeds valid range (-999999 to 999999)");
        return;
    }
...
void Simpletron::DIVIDE(int operand) {
    // Cant access Negative memory
    if (!checkMemoryBounds(operand)) {
        opcodeError(34, "Memory bounds violation",
                    "Operand " + to_string(operand) + " is out of memory bounds (0-9999)");
        return;
    }
    // Division by zero
    if (this->memory[operand] != 0) {
        ...
        opcodeError(34, "Division by zero",
                    "Cannot divide by zero at memory location " + to_string(operand));
    }
}
...
void Simpletron::DIVIDEX() {
    // Cant access Negative memory
    if (!checkMemoryBounds(this->IndexRegister)) {
        opcodeError(35, "IndexRegister bounds violation",
                    "IndexRegister value " + to_string(this->IndexRegister) +
                        " is out of memory bounds (0-9999)");
        return;
    }
    ...
    if (this->memory[this->IndexRegister] != 0) {
        ...
        opcodeError(35, "Division by zero",
                    "Cannot divide by zero at memory location " + to_string(this->IndexRegister));
    }
}
...
void Simpletron::MULTIPLY(int operand) {
    // Cant access Negative memory
    if (!checkMemoryBounds(operand)) {
        opcodeError(36, "Memory bounds violation",
                    "Operand " + to_string(operand) + " is out of memory bounds (0-9999)");
        return;
    }
    // Overflow
    if (checkOverflow(this->accumulator * this->memory[operand])) {
        opcodeError(36, "Arithmetic overflow",
                    "Result " + to_string(this->accumulator * this->memory[operand]) +
                        " exceeds valid range (-999999 to 999999)");
        return;
    }
...
void Simpletron::MULTIPLYX() {
    // Cant access Negative memory
    if (!checkMemoryBounds(this->IndexRegister)) {
        opcodeError(37, "IndexRegister bounds violation",
                    "IndexRegister value " + to_string(this->IndexRegister) +
                        " is out of memory bounds (0-9999)");
        return;
    }
    // Overflow
    if (checkOverflow(this->accumulator * this->memory[this->IndexRegister])) {
        opcodeError(37, "Arithmetic overflow",
                    "Result " + to_string(this->accumulator * this->memory[this->IndexRegister]) +
                        " exceeds valid range (-999999 to 999999)");
        return;
    }
...
void Simpletron::printMemoryPageHeader(int pageNumber) {
    cout << endl;
    cout << "PAGE # " << setfill('0') << setw(2) << pageNumber << endl << endl;
...
void Simpletron::printRegisters() {
    ...
    if (this->accumulator > 0) {
        ...
        cout << "accumulator          " << setfill('0') << setw(6) << this->accumulator << endl;
    }

    // Print other registers
    cout << "InstructionCounter   " << setfill('0') << setw(6) << this->InstructionCounter << endl;
    cout << "IndexRegister        " << setfill('0') << setw(6) << this->IndexRegister << endl;
